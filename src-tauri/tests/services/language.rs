...
#[rstest]
#[tokio::test]
async fn test_error_on_creating_duplicate_language(create_language_payload: EntryCreateSchema) {
    let db = database().await;
...
#[rstest]
#[tokio::test]
async fn test_delete_language(create_language_payload: EntryCreateSchema) {
    let db = database().await;
