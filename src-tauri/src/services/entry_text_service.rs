...
pub async fn sync_text(database: &DatabaseConnection, text: &String) -> TextNode {
    let mut text = _parse_text(&text);

    let mut text_errors: Vec<TextError> = Vec::new();
...
fn _parse_text(text: &str) -> TextNode {
    let text_result: Result<TextNode, serde_json::Error> = serde_json::from_str(&text);
    // TODO: check how empty strings are handled here
    match text_result {
        Ok(text) => text,
        Err(_) => TextNode::new_doc(), // TODO: log error
    }
}
...
async fn _sync_text_node(
    ...
    node: &mut TextNode,
    text_errors: &mut Vec<TextError>,
    db_errors: &mut Vec<DbErr>,
) {
    if node.is_type("mention") {
        _sync_reference_label(database, node, text_errors, db_errors).await;
    }
...
async fn _sync_reference_label(
    ...
    if let Some(ref_id_value) = ref_id_option {
        if let Some(ref_id) = ref_id_value.as_i64() {
            ...
            match entry_result {
                Ok(optional_entry) => match optional_entry {
                    ...
                    None => {
                        ...
                        text_errors.push(TextError::bad_reference_id(
            ...
            text_errors.push(TextError::bad_value_type(
        ...
        text_errors.push(TextError::missing_attr(
