use sea_orm::*;

use ::entity::{word, word::Entity as Word};

use crate::types::{
    CodedType, GrammaticalGender, GrammaticalNumber, GrammaticalPerson, VerbForm, VerbTense,
    WordType,
};

use super::util;

pub async fn insert(
    db: &DbConn,
    language_id: i32,
    word_type: WordType,
    spelling: &str,
    number: GrammaticalNumber,
    person: GrammaticalPerson,
    gender: GrammaticalGender,
    verb_form: VerbForm,
    verb_tense: VerbTense,
    translations: serde_json::Value,
) -> Result<word::Model, DbErr> {
    let new_entity = word::ActiveModel {
        id: NotSet,
        language_id: Set(language_id),
        word_type: Set(word_type.code()),
        spelling: Set(spelling.to_string()),
        number: Set(number.code()),
        person: Set(person.code()),
        gender: Set(gender.code()),
        verb_form: Set(verb_form.code()),
        verb_tense: Set(verb_tense.code()),
        translations: Set(translations),
    };
    return new_entity.insert(db).await;
}

pub async fn update(
    db: &DbConn,
    id: i32,
    language_id: Option<i32>,
    word_type: Option<WordType>,
    spelling: Option<String>,
    number: Option<GrammaticalNumber>,
    person: Option<GrammaticalPerson>,
    gender: Option<GrammaticalGender>,
    verb_form: Option<VerbForm>,
    verb_tense: Option<VerbTense>,
    translations: Option<serde_json::Value>,
) -> Result<word::Model, DbErr> {
    let Some(existing_entity) = get(db, id).await? else {
        return Err(DbErr::RecordNotFound("Word not found.".to_owned()));
    };
    let translations = match translations {
        Some(t) => Set(t),
        None => NotSet,
    };
    let updated_entity = word::ActiveModel {
        id: Unchanged(existing_entity.id),
        language_id: util::optional_value_to_active_value(language_id),
        word_type: util::optional_type_to_active_value(word_type),
        spelling: util::optional_value_to_active_value(spelling),
        number: util::optional_type_to_active_value(number),
        person: util::optional_type_to_active_value(person),
        gender: util::optional_type_to_active_value(gender),
        verb_form: util::optional_type_to_active_value(verb_form),
        verb_tense: util::optional_type_to_active_value(verb_tense),
        translations,
    };
    updated_entity.update(db).await
}

pub async fn get(db: &DbConn, id: i32) -> Result<Option<word::Model>, DbErr> {
    Word::find_by_id(id).one(db).await
}

pub async fn get_multiple(
    db: &DbConn,
    language_id: i32,
    word_type: Option<WordType>,
) -> Result<Vec<word::Model>, DbErr> {
    let mut query = Word::find().filter(word::Column::LanguageId.eq(language_id));
    if word_type.is_some() {
        query = query.filter(word::Column::WordType.eq(word_type.unwrap().code()));
    }
    query.order_by_asc(word::Column::Spelling).all(db).await
}

pub async fn delete(db: &DbConn, id: i32) -> Result<DeleteResult, DbErr> {
    let Some(existing_entity) = get(db, id).await? else {
        return Err(DbErr::RecordNotFound("Word not found.".to_owned()));
    };
    return existing_entity.delete(db).await;
}
